  ...
  return (
    <ol className={cn("space-y-3", className)} aria-label="Lead source and edit history">
      {history.map((entry) => {
        ...
        const changedFieldLabels = (entry.changedFields || []).map(
          (field) => EDIT_FIELD_LABELS[field] || field
        );
        ...
        return (
          ...
          >
            ...
            <div className="min-w-0">
              ...
              <p className="mt-2 text-xs leading-5 text-zinc-600">
                {isProfileEdit
                  ? changedFieldLabels.length
                    ? `${actor} updated ${changedFieldLabels.length} lead field${changedFieldLabels.length === 1 ? "" : "s"}.`
              ...
              {isProfileEdit && (changedFieldLabels.length || entry.editVersion) ? (
                <div className="mt-2 flex flex-wrap gap-1.5" aria-label="Edited fields">
                  {entry.editVersion ? (
                    <span className="rounded-full border border-violet-200 bg-violet-50 px-2 py-0.5 text-[10px] font-medium text-violet-700">
                      Version {entry.editVersion}
                    </span>
                  ) : null}
                  {changedFieldLabels.map((label) => (
                    <span key={label} className="rounded-full border border-zinc-200 bg-zinc-50 px-2 py-0.5 text-[10px] font-medium text-zinc-600">
                      {label}
                    </span>
                  ))}
                </div>
