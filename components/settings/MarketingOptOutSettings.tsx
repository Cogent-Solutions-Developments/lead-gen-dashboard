...
import { ArrowLeft, RefreshCw, ShieldOff, UploadCloud } from "lucide-react";
...
export function MarketingOptOutSettings({ onBack }: { onBack: () => void }) {
  ...
  const loadOptOutList = useCallback(async () => {
    ...
    try {
      ...
      toast.error("Failed to load opt-out list", { description: getErrorMessage(error) });
      setOptOutRows([]);
    } finally {
  ...
  const handleOptOutCsvUpload = async () => {
    ...
    try {
      ...
      toast.error("CSV upload failed", { description: getErrorMessage(error) });
    } finally {
  ...
  const handleManualAdd = async () => {
    ...
    try {
      ...
      toast.error("Failed to save suppression", { description: getErrorMessage(error) });
    } finally {
      ...
      <Card className="p-6">
        <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="mb-1 text-xs font-medium text-slate-400">Settings / Marketing Opt-out</p>
          ...
          <Button type="button" variant="outline" onClick={() => void loadOptOutList()} disabled={optOutLoading || uploadingCsv || addingManual} className="border-slate-300 text-slate-700">
        ...
        <div className="mb-3 grid gap-3 rounded-lg border border-slate-300 bg-slate-50/60 p-4 md:grid-cols-3">
          <div className="space-y-1">
            <label className="block text-sm font-medium text-slate-700" htmlFor="opt-out-phone">Phone (optional)</label>
            <Input id="opt-out-phone" value={manualPhone} onChange={(event) => setManualPhone(event.target.value)} placeholder="+9477xxxxxxx" disabled={addingManual} />
          </div>
          <div className="space-y-1">
            <label className="block text-sm font-medium text-slate-700" htmlFor="opt-out-email">Email (optional)</label>
            <Input id="opt-out-email" value={manualEmail} onChange={(event) => setManualEmail(event.target.value)} placeholder="contact@example.com" disabled={addingManual} />
          </div>
          <div className="space-y-1">
            <label className="block text-sm font-medium text-slate-700" htmlFor="opt-out-reason">Reason (optional)</label>
            <Input id="opt-out-reason" value={manualReason} onChange={(event) => setManualReason(event.target.value)} placeholder="requested no marketing" disabled={addingManual} />
          </div>
          <div className="md:col-span-3">
            <Button type="button" onClick={() => void handleManualAdd()} disabled={addingManual} className="w-full md:w-auto">
        ...
        <div className="grid gap-3 rounded-lg border border-slate-300 bg-slate-50/60 p-4 md:grid-cols-[1fr_auto] md:items-end">
          <div className="space-y-1">
            <label className="block text-sm font-medium text-slate-700" htmlFor="opt-out-csv">Upload Opt-out CSV</label>
            <Input id="opt-out-csv" type="file" accept=".csv,text/csv" onChange={(event) => setSelectedCsv(event.target.files?.[0] ?? null)} disabled={uploadingCsv} />
            <p className="text-xs text-slate-500">Supported columns: <span className="font-mono">phone</span>, <span className="font-mono">mobile</span>, <span className="font-mono">number</span>, <span className="font-mono">email</span>.</p>
          </div>
          <Button type="button" onClick={() => void handleOptOutCsvUpload()} disabled={!selectedCsv || uploadingCsv}>
            {uploadingCsv ? <RefreshCw className="mr-2 h-4 w-4 animate-spin" /> : <UploadCloud className="mr-2 h-4 w-4" />}
            Upload CSV
          </Button>
        </div>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          <label className="inline-flex items-center gap-2 text-sm text-slate-700">
            <input type="checkbox" checked={activeOnly} onChange={(event) => setActiveOnly(event.target.checked)} className="h-4 w-4 rounded border-slate-300 text-slate-900 focus:ring-slate-900" />
            Show active only
          </label>
        ...
        {uploadSummary ? (
          <div className="mt-4 grid gap-2 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-xs text-emerald-800 sm:grid-cols-2 lg:grid-cols-6">
            <span>Effective rows: {uploadSummary.effectiveRows}</span><span>Phone rows: {uploadSummary.phoneRows}</span><span>Email rows: {uploadSummary.emailRows}</span><span>Created: {uploadSummary.created}</span><span>Updated: {uploadSummary.updated}</span><span>Invalid: {uploadSummary.invalid}</span>
          </div>
        ) : null}
        ...
        <div className="mt-4 overflow-x-auto rounded-lg border border-slate-300">
          <table className="min-w-full text-left text-sm">
            <thead className="bg-slate-100/80 text-xs uppercase tracking-wide text-slate-600">
              <tr><th className="px-3 py-2">Scope</th><th className="px-3 py-2">Identity</th><th className="px-3 py-2">Phone</th><th className="px-3 py-2">Email</th><th className="px-3 py-2">Source</th><th className="px-3 py-2">Reason</th><th className="px-3 py-2">Created</th></tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {optOutRows.length === 0 ? (
                <tr><td colSpan={7} className="px-3 py-6 text-center text-slate-500">{optOutLoading ? "Loading opt-out records..." : "No opt-out records found."}</td></tr>
              ) : (
                optOutRows.map((row) => (
