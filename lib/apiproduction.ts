...
export async function listEvents(params?: { leadGroup?: import("@/lib/leads/leadTypes").LeadGroup }) {
  const { data } = await apiClientProduction.get<EventSummaryResponse>("/api/productions/events", { params });
  return data;
}
...
export async function generateLeadContent(id: string, payload: LeadContentGenerationRequest) {
  return generateDurablePreview<LeadContentGenerationResponse>(apiClientProduction, `/api/productions/leads/${id}/content/previews`, payload);
}
...
export async function sendSelectedCampaignLeads(payload: SendSelectedLeadsRequest) {
  ...
  const { data } = await apiClientProduction.post<SendSelectedLeadsResponse>(
    `/api/productions/campaigns/${campaignId}/${endpoint}${queryText ? `?${queryText}` : ""}`,
    leadIds
  );
  return data;
}
