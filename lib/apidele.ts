...
export async function validateLeadTemplateUpload(file: File | Blob, eventRegistryId?: string) {
  ...
  const { data } = await apiClientDelegate.post<LeadTemplateValidationResponse>(
    "/api/delegates/campaigns/lead-template/validate",
    formData
  );
  return data;
}
...
export async function downloadLeadTemplateFile(fileName = "lead-upload-template.xlsx") {
  const { data } = await apiClientDelegate.get<Blob>("/api/delegates/campaigns/lead-template", {
    responseType: "blob",
  });
...
export async function searchLeads(params?: GlobalLeadSearchParams) {
  const { data } = await apiClientDelegate.get<GlobalLeadSearchResponse>("/api/delegates/leads/search", {
    params: {
      limit: params?.limit,
      offset: params?.offset,
      search: params?.search,
...
export async function getLeadOwnerHistory(id: string) {
  const { data } = await apiClientDelegate.get<LeadOwnerHistoryResponse>(
    `/api/delegates/leads/${id}/owner-history`
  );
  return data;
}
