...
async function fetchConfirmedLeadsForEvent(baseUrl: string, apiKey: string, eventKey: string) {
  ...
  while (true) {
    const url = `${baseUrl}/api/productions/events/${encodeURIComponent(eventKey)}/leads?includeManual=true&limit=${limit}&offset=${offset}`;
...
export async function GET(request: NextRequest) {
  ...
  if (!baseUrl || !apiKey) {
    return NextResponse.json({ detail: "Production KPI API is not configured." }, { status: 500 });
  }
  ...
  try {
    ...
    const productionUsers = (Array.isArray(usersResponse.users) ? usersResponse.users : [])
      .filter((user) => String(user.role || "").trim() === "production_user" && (user.isActive ?? user.is_active) !== false)
      .map((user) => {
    ...
    const counts = new Map(productionUsers.map((user) => [user.id, 0]));
    const eventsResponse = await fetchJson<{ events?: BackendEvent[] }>(`${baseUrl}/api/productions/events`, apiKey);
    ...
    const detail = error instanceof Error ? error.message : "Failed to load production KPI.";
